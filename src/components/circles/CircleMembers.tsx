...
const CircleMembers: React.FC<CircleMembersProps> = ({ circle, isOwner }) => {
  ...
  return (
    <div className="px-4 py-6">
      {isOwner ? (
        ...
        /* Member View - Only show count */
        <div className="text-center py-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 mb-4">
            <UserPlus className="h-8 w-8 text-primary" />
          </div>
          <h3 className="text-lg font-semibold mb-2">
            {circle?.members_count || mockMembers.length} Members
          </h3>
          <p className="text-sm text-muted-foreground">
            Join this circle to see all members and connect with the community
          </p>
        </div>
      )}
    </div>
